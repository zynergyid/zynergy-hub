...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   ALTER TYPE "public"."enum_events_kind" ADD VALUE IF NOT EXISTS 'mentoring' BEFORE 'konten';`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_events_kind" AS ENUM('rapat-tim', 'meeting-klien', 'konten', 'fokus', 'lainnya');
