...
export async function saveTransaction(_prev: QuickAddState, formData: FormData): Promise<QuickAddState> {
  ...
  if (!canWriteUnit(user, unit, "editMoney")) return { status: "error", message: "Anda tidak punya akses ke unit ini." };
  ...
  try {
    ...
    if (id) {
      ...
      if (!existing || !canWriteUnit(user, existing.unit, "editMoney")) {
...
export async function deleteTransaction(formData: FormData) {
  ...
  if (!existing || !canWriteUnit(user, existing.unit, "editMoney")) return;
