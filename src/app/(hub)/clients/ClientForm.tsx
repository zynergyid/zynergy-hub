...
import { businessTypes, clientKinds, clientStatuses, packages, units, type ClientKind, type Unit } from "@/lib/options";
...
/**
 * The first block asks only what a new client needs (who, how to reach them).
 * Everything else folds under "Lengkapi data lain", open by default when editing.
 */
export function ClientForm({
  ...
  const [kind, setKind] = useState<ClientKind>(client?.kind ?? "usaha");
  ...
  // One person is the client: no PIC, no business sector, no legal entity.
  const person = kind === "perorangan";
  const supply = client?.supply;
  const namePlaceholder = person ? "Contoh: Rina Kartika" : isSupply ? "Contoh: PT Tambang Nusantara" : "Contoh: Warung Nasi Uduk Pak Wawan";
    ...
    <form action={formAction} className="space-y-5 rounded-2xl border border-line bg-white p-5 shadow-[0_1px_2px_rgba(15,27,51,0.04)] sm:p-6">
      ...
      <fieldset disabled={readOnly} className="space-y-5 disabled:opacity-90">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <Label htmlFor="cf-kind">Bentuk klien</Label>
            <Select id="cf-kind" name="kind" value={kind} onValueChange={(v) => setKind(v as ClientKind)} options={clientKinds} />
          </div>
          <div className="sm:col-span-2">
            <Label htmlFor="cf-name">{person ? "Nama" : isSupply ? "Nama perusahaan" : "Nama usaha"}</Label>
            <input id="cf-name" name="name" required defaultValue={client?.name} className={fieldClass} placeholder={namePlaceholder} />
          </div>
          {!person && (
            <div>
        ...
        <details open={Boolean(client)} className="group rounded-xl border border-line">
